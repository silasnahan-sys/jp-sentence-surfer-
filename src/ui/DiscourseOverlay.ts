...
export class DiscourseOverlay {
    ...
    private renderOverlay(view: MarkdownView): void {
        ...
        const markers = detectPatternsInText(text);
    ...
    private appendChip(
        ...
        try {
            const startPos = view.editor.offsetToPos(marker.charStart);
            const coords = (view.editor as any).coordsAtPos?.(startPos);
            if (!coords) return;

            const chip = document.createElement('span');
            chip.classList.add('jp-surfer-discourse-overlay-chip');
            chip.style.position = 'absolute';
            chip.style.left = `${coords.left}px`;
            chip.style.top = `${coords.top - 20}px`;
            chip.style.backgroundColor = marker.color + '22';
            chip.style.borderColor = marker.color;
            chip.style.color = marker.color;
            chip.textContent = marker.label;
            chip.title = `${marker.text} (${marker.subcategory}) conf:${Math.round(marker.confidence * 100)}%`;

            chip.addEventListener('click', () => {
                view.editor.setCursor(view.editor.offsetToPos(marker.charStart));
            });
